...
- データベースの設計、モデルのリレーション、コントローラーの実装といったバックエンド開発の基本を習得する。
...
| 機能分類 | 機能名 | 詳細な仕様 |
| :--- | :--- | :--- |
| **ユーザー向け機能** | お問い合わせフォーム | - 氏名、性別、メールアドレス、電話番号、住所、建物名（任意）、お問い合わせの種類、タグ（複数選択可）、お問い合わせ内容を入力できる。<br>- 入力内容の確認画面を表示できる。<br>- バリデーション（入力チェック）機能がある。<br>- フォームを送信すると、データベースにお問い合わせ内容が保存される。 |
| | お問い合わせ完了画面 | - お問い合わせ送信後、「お問い合わせありがとうございました」というメッセージを表示する。 |
| **管理者向け機能** | ログイン機能 | - 管理者専用のログイン画面からログインできる。<br>- 認証されていないユーザーは管理画面にアクセスできない。 |
| | 管理画面 | - 登録されたお問い合わせを一覧で表示する（ページネーション付き）。<br>- キーワード、性別、お問い合わせの種類、日付で絞り込み検索ができる。<br>- 各お問い合わせの詳細をモーダルウィンドウで表示できる。<br>- 特定のお問い合わせを削除できる。<br>- タグの管理（追加・更新・削除）ができる。 |
...
| `tags` | `id` | `BIGINT` (PK) | タグID |
| | `name` | `VARCHAR` | タグ名（最大50文字、ユニーク） |
| `contact_tag` | `contact_id` | `BIGINT` (FK) | `contacts`テーブルと紐づく外部キー |
| | `tag_id` | `BIGINT` (FK) | `tags`テーブルと紐づく外部キー |
...
### Point 5: サーバーサイドレンダリング (SSR) で画面を組み立てる

今回のシステムでは、**サーバーサイドレンダリング（SSR）** 方式を採用しています。ユーザーからのリクエストをサーバー（Laravel）が受け取り、コントローラーで処理した結果を**Bladeテンプレート**に渡してHTMLを組み立て、ブラウザに返します。つまり、サーバー側で画面の完成形を作ってから送り出す方式です。フロントエンドとバックエンドが一つのLaravelプロジェクト内で完結するため、構成がシンプルで見通しが良く、お問い合わせフォームのような比較的シンプルなWebアプリケーションには最適なアーキテクチャです。
...
| `resources/views/contact/_form.blade.php` | お問い合わせフォームの入力部品（`@include`で読み込まれる） |
...
開発を始める前に、プロジェクトの「顔」となる **README.md** について理解しておきましょう。README.mdは、採点者やチームメンバーがプロジェクトを初めて見たときに最初に読むファイルです。環境構築の手順やアプリケーションの概要が正確に記載されていなければ、そもそもアプリケーションを動かして評価することができません。

### 記載必須項目

README.md には、以下の項目を必ず記載してください。

| 項目 | 説明 |
| :--- | :--- |
| **プロジェクト名** | アプリケーションの名称（例: お問い合わせ管理システム） |
| **概要** | アプリケーションの目的や主な機能の概要 |
| **ER図** | データベースのテーブル構造とリレーションを示す図 |
| **環境構築手順** | ローカル環境でアプリケーションを動かすまでの手順（`git clone` から `sail up` まで） |
| **使用技術** | 使用している言語・フレームワーク・ツールとそのバージョン（例: PHP 8.2、Laravel 10.x、MySQL） |
| **作成者** | 開発者の名前 |

### ER図の作成

ER図（Entity-Relationship Diagram）は、データベースのテーブル同士の関係を視覚的に表現した図です。Chapter 2 でマイグレーションを実装した後に作成し、README.md に添付します。

ER図の作成方法は主に2つあります。

**方法1: Mermaid記法で記述する（推奨）**

GitHubのREADME.mdでは、Mermaid記法でER図を直接描画できます。
...
draw.io などのツールで作成したER図を画像ファイルとしてリポジトリに保存し、README.md から参照します。
...
## 概要
お問い合わせフォームから送信された内容を管理者が閲覧・検索できるWebアプリケーションです。
...
> **📌 重要: ER図はコーチに提出し、承認を得てから実装に進んでください。** データベース設計はアプリケーション全体の土台です。Chapter 2 でマイグレーションを実装する前に、ER図をコーチにレビューしてもらい、設計の方向性が正しいことを確認してから次のステップに進みましょう。
...
- **どうやって（How）**: LaravelのMVCアーキテクチャに則り、データベース、モデル、コントローラー、ルーティングなどを段階的に実装していく。
