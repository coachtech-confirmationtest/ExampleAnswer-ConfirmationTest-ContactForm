...
**1. 認証失敗メッセージ（`lang/ja/auth.php`）**

メールアドレスやパスワードの入力形式は正しいが、該当するユーザーがデータベースに存在しない、またはパスワードが間違っている場合に表示されるメッセージです。これはバリデーションエラーではなく、認証（ログイン）処理そのものの失敗メッセージです。

`lang/ja/auth.php`を新規作成し、以下の内容を記述します。
...
**2. バリデーションメッセージ（`lang/ja/validation.php`）**

メールアドレスやパスワードが未入力、またはメールアドレスの形式が正しくない場合に表示されるメッセージです。こちらは入力値の形式チェック（バリデーション）のエラーメッセージです。

`lang/ja/validation.php`を新規作成し、以下の内容を記述します。
