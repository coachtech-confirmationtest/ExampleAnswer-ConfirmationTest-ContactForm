...
> **⚠️ 注意: 新しいターミナルタブを開いてから実行してください。**
>
> `sail npm run dev` はViteの開発サーバーを**常駐プロセス**として起動するため、実行中はそのターミナルが占有されます。今後も他のコマンド（`sail artisan` など）を実行する必要があるので、**別のターミナルタブ（またはウィンドウ）を開いてから**以下のコマンドを実行してください。
...
環境構築が完了したら、ソースコードをバージョン管理するために **Git** の設定を行います。Gitを使うことで、コードの変更履歴を記録し、いつでも過去の状態に戻ることができます。
...
# 初回コミット
git commit -m "feat: Laravelプロジェクトの初期セットアップ"
```

#### GitHub リポジトリの作成とリモート接続

1. GitHubで新しいリポジトリを作成します（README.mdの自動生成はチェックを外してください）。
...
開発では、**機能ごとにブランチを作成**し、完成したら `main` ブランチにマージします。`main` ブランチに直接コミットするのではなく、作業用のブランチを切って開発を進めることで、安定したコードを保つことができます。

```bash
# 新しいブランチを作成して切り替え
git checkout -b feature/create-migration

# 作業が完了したらmainにマージ
git checkout main
git merge feature/create-migration
```

#### コミットメッセージ規約

コミットメッセージは **「何をしたか」が明確にわかるように** 記述します。以下の **Conventional Commits** 形式に従ってください。

| プレフィックス | 用途 | 例 |
| :--- | :--- | :--- |
| `feat:` | 新しい機能の追加 | `feat: お問い合わせ一覧画面を実装` |
| `fix:` | バグの修正 | `fix: バリデーションエラーが表示されない問題を修正` |
| `refactor:` | 機能変更を伴わないコードの改善 | `refactor: コントローラーのクエリをスコープに移動` |

具体的なコミットメッセージの例:

```
feat: マイグレーションファイルを作成
feat: お問い合わせフォームのバリデーションを実装
feat: 管理画面の検索機能を実装
fix: 電話番号のバリデーションルールを修正
refactor: N+1問題をEagerLoadingで解消
```

> **💡 ポイント**: コミットはこまめに行いましょう。一つの機能が完成するたびにコミットしておけば、問題が発生したときに直前の正常な状態に戻すことができます。
