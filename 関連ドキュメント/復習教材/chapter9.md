...
ルーティングの実装が完了したことで、アプリケーション全体が初めてブラウザ上で動作するようになります。以下の動作を確認してください。

**お問い合わせフォーム（公開ページ）**
1. `http://localhost/`にアクセスし、お問い合わせ入力フォームが表示されること
2. フォームに必要事項を入力して送信し、確認画面が表示されること
3. 確認画面で「送信」を押し、サンクスページが表示されること
4. 入力内容に不備がある場合、バリデーションエラーメッセージが表示されること

**管理画面（認証必須ページ）**
5. 未ログイン状態で`http://localhost/admin`にアクセスすると、ログインページにリダイレクトされること
6. ログイン後、管理画面にお問い合わせ一覧が表示されること
