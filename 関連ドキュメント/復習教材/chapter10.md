# Chapter 10: 管理画面の心臓部 - CSVエクスポート機能を実装する

## 1. はじめに 📖

お疲れ様です！ここからは応用機能です。最初に取り組むのは、多くのWebアプリケーションで必須とも言える「CSVエクスポート機能」です。管理画面に蓄積されたお問い合わせデータを、CSVファイルとしてダウンロードできるようにします。

これまでのチャプターでは、コントローラーでデータを処理し、Bladeテンプレートを通じてHTMLを返す方法を学んできました。今回はそれとは少し異なり、ブラウザから直接ファイルをダウンロードさせるアプローチを取ります。この機能により、ユーザーは使い慣れた表計算ソフト（Excelなど）でデータを自由に分析・加工できるようになります。
...
### Point 1: ファイルダウンロードはどこに書く？

「今回はBladeで画面を返すんじゃなくて、ブラウザに『このファイルをダウンロードしてね』って命令する必要がある。これまでと同じく`routes/web.php`にルートを定義して、`ContactController`に処理を書けばいいな。レスポンスの種類が違うだけで、ルーティングの考え方は同じだ。」
...
「今回は`get()`で全件取得してからCSVに書き出す方法で実装しよう。今回のプロジェクトのデータ量なら問題ないはずだ。でも、実務では何十万件ものデータを扱う可能性がある。その場合は`get()`だとメモリを使い果たす危険があるから、`chunk()`や`cursor()`を使って少しずつ処理するのが鉄則だ。今回は`get()`で実装するけど、`chunk()`を使う方法も頭の片隅に置いておこう。」
...
```bash
sail artisan make:request ExportContactRequest
```
...
CSVエクスポート機能が正しく動作するか、以下を確認してください。

1. 管理画面（`http://localhost/admin`）にログインし、「エクスポート」ボタンを押してCSVファイルがダウンロードされること
2. ダウンロードされたファイル名に日時が含まれていること（例: `contacts_20250101_120000.csv`）
3. CSVファイルをExcelで開き、文字化けせずに日本語が正しく表示されること
4. 検索条件を指定した状態でエクスポートし、検索結果のみがCSVに含まれていること
...
**学んだ重要なポイント:**
- **Webルートとコントローラー**: ファイルダウンロードのようなブラウザ固有の機能も、これまでと同じWebルートとコントローラーで実装できることを学びました。
...
これで、確認テストで求められるCSVエクスポート機能の実装が完了しました。次のチャプターでは、もう一つの応用機能として**公開APIの提供**に挑戦します。これまでBlade（SSR）で構築してきたアプリケーションに、JSON形式でデータを返すAPIエンドポイントを追加し、API Resources、APIコントローラー、APIルーティングの書き方を学んでいきます。
