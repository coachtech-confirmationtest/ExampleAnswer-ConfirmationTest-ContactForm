...
| | `confirm` | POST | `/contacts/confirm` | バリデーション済みデータで確認画面を表示する（タグ名も表示） |
| | `store` | POST | `/contacts` | お問い合わせ情報を保存し、タグを紐付けて完了画面へリダイレクトする |
| | `thanks` | GET | `/thanks` | 送信完了画面を表示する |
| `AdminController` | `index` | GET | `/admin` | 管理画面を表示する（検索・ページネーション付き、タグのEager Loading含む） |
| | `show` | GET | `/admin/contacts/{contact}` | お問い合わせの詳細画面を表示する（タグ情報含む） |
| | `destroy` | DELETE | `/admin/contacts/{contact}` | お問い合わせを削除し、管理画面へリダイレクトする |
| `TagController` | `edit` | GET | `/admin/tags/{tag}/edit` | タグの編集画面を表示する |
| | `store` | POST | `/admin/tags` | 新しいタグを作成し、管理画面へリダイレクトする |
...
- **役割ごとの分離**: `ContactController`はエンドユーザー向けのお問い合わせフォーム、`AdminController`は管理者向けのお問い合わせ管理と、それぞれが異なる関心事を担当しています。**3つのコントローラー体制**: `ContactController`（エンドユーザー向け）、`AdminController`（管理者向けデータ管理）、`TagController`（タグのCRUD操作）の3つがそれぞれ異なる関心事を担当しています。
...
- **`ContactController`**: エンドユーザー向けのお問い合わせフォームに関する処理（フォーム表示、確認、保存、完了画面）
- **`AdminController`**: 管理者向けのお問い合わせ管理に関する処理（一覧表示・検索、詳細表示、削除）

このように、それぞれのコントローラーが担当する「関心事」を明確に分離することが重要です。これにより、コードが読みやすくなり、変更が必要になった際に影響範囲を特定しやすくなります。これは、ソフトウェア設計における**単一責任の原則**の実践です。タグのマスタデータ管理を担う`TagController`も同様の考え方で設計しています。
...
### Point 4: `attach()`で多対多リレーションのデータを紐付ける

お問い合わせとタグを紐付ける際、`DB::table('contact_tag')->insert(...)`のように手動で中間テーブルを操作することも可能ですが、Eloquentのリレーションメソッドを使えば`$contact->tags()->attach($tagIds)`という一行で直感的に実現できます。`attach()`は中間テーブルに新しいレコードを追加するメソッドです。

### Point 5: フォーム送信後はリダイレクトで画面を更新する（PRGパターン）

伝統的なWebアプリケーションでは、フォーム送信（POST/PUT/DELETE）の後にリダイレクトを行うのが定石です。これは「POST/Redirect/GET（PRG）パターン」と呼ばれ、ブラウザのリロードによるフォームの二重送信を防ぐための重要なテクニックです。

### Point 6: HTMLフォームで`PUT`や`DELETE`メソッドを使うには`@method`ディレクティブが必要

HTMLの`<form>`タグは`GET`と`POST`の2つのHTTPメソッドしかサポートしていません。Laravelでは、Bladeテンプレートの`@method('PUT')`や`@method('DELETE')`ディレクティブを使うことで、隠しフィールドを生成してこの制約を回避できます。`@csrf`ディレクティブも必ず含める必要があります。
...
このコントローラーは、エンドユーザー向けのお問い合わせフォームの一連のフローを担当します。フォーム表示、確認画面、保存処理、完了画面を提供します。
...
タグの編集画面表示・作成・更新・削除を処理するコントローラーです。
...
管理画面では、タグの新規作成フォームとタグ一覧（テキスト表示 + 編集リンク + 削除ボタン）を表示します。削除ボタンには`@method('DELETE')`を使って`DELETE`メソッドを擬似的に送信します。

#### タグ編集ページ（`admin/tags/edit.blade.php`）: 更新フォーム

タグの編集は専用ページで行います。`@method('PUT')`で`PUT`メソッドを擬似的に実現し、`old('name', $tag->name)`でバリデーションエラー時の値保持と初回表示時の現在値表示を両立します。
...
- **`$categories = Category::all();`**: フォームのセレクトボックスに表示するためのカテゴリーデータを、データベースから全件取得します。
- **`return view('contact.index', compact('categories'));`**: `view()`ヘルパーで`resources/views/contact/index.blade.php`テンプレートを返します。`compact()`関数で、変数名をキーとして連想配列に変換し、Bladeテンプレートに渡しています。テンプレート内では`$categories`としてアクセスできます。
...
- **`return view('contact.confirm', compact('validated', 'category'));`**: 確認画面のBladeテンプレートにバリデーション済みデータとカテゴリーを渡して表示します。

#### `store`メソッド（保存処理）

- **`Contact::create($validated);`**: バリデーション済みのデータで新しい`Contact`レコードを作成します。`create`メソッドを使うには、`Contact`モデル側で`$fillable`プロパティが正しく設定されている必要があります。
...
- **`$categories = Category::all();`**: 検索フォームのセレクトボックスに使うカテゴリーデータも取得します。
- **`return view('admin.index', compact('contacts', 'categories'));`**: 検索結果とフォーム用データをBladeテンプレートに渡してHTMLを返します。

#### `show`メソッド（詳細表示）

- **`public function show(Contact $contact)`**: ルートモデルバインディングにより、URLのIDに対応する`Contact`モデルが自動的にインジェクトされます。
- **`$contact->load('category');`**: 既に取得済みのモデルに対してリレーションを後から読み込む（Lazy Eager Loading）メソッドです。詳細画面でカテゴリー名を表示するために必要です。
...
### `ContactController` のタグ関連処理

- **`$tags = Tag::all();`（`index`メソッド）**: タグのチェックボックスをフォームに表示するために、全タグを取得してビューに渡します。
- **`$tags = isset($validated['tag_ids']) ? Tag::whereIn('id', $validated['tag_ids'])->get() : collect();`（`confirm`メソッド）**: 選択されたタグIDからタグモデルを取得し、確認画面でタグ名を表示します。選択されていない場合は空のコレクションを返します。
- **`$tagIds = $validated['tag_ids'] ?? [];`（`store`メソッド）**: タグIDを別変数に退避します。
- **`unset($validated['tag_ids']);`**: `Contact::create()`に`tag_ids`を渡すとエラーになるため、事前に削除します。
- **`$contact->tags()->attach($tagIds);`**: `attach()`メソッドで中間テーブルにタグを紐付けます。

### `AdminController` のEager LoadingとN+1問題

- **`Contact::with(['category', 'tags'])`**: `paginate`を呼び出す前に`with()`で`category`と`tags`のリレーションをEager Loadingしています。これにより、10件のお問い合わせがあっても追加クエリは2回（categoriesとtags）で済みます。Eager Loadingしないと1+10回のクエリ（N+1問題）が発生します。
- **`$contact->load(['category', 'tags']);`**: `show`メソッドでは`load()`でLazy Eager Loadingを行い、詳細画面でタグ情報も表示します。

### `TagController` の解説

- **`edit(Tag $tag)`**: ルートモデルバインディングで`Tag`モデルが自動注入されます。
- **`store(StoreTagRequest $request)`**: `StoreTagRequest`によるバリデーション後、`Tag::create()`で新しいタグを作成し、管理画面にリダイレクトします。
- **`update(UpdateTagRequest $request, Tag $tag)`**: `UpdateTagRequest`でユニーク制約の自分自身除外を含むバリデーションを行い、更新します。
- **`destroy(Tag $tag)`**: タグを削除します。カスケード削除により、中間テーブルの関連レコードも自動削除されます。
- **`redirect('/admin')`**: すべてのメソッドがPRGパターンに従い、処理後に管理画面にリダイレクトします。

### 💡 コラム: `@csrf`と`@method`の仕組み

Bladeテンプレートで使う`@csrf`と`@method`は、それぞれ隠しフィールドを生成します。`@csrf`はCSRF攻撃を防ぐトークンを、`@method('PUT')`はHTMLフォームの制約（GETとPOSTのみ）を回避するための`_method`フィールドを生成します。Laravelのルーターはこのフィールドを検知し、実際のHTTPメソッドとして扱います。
...
- **責務の分離**: エンドユーザー向けの`ContactController`と、管理者向けの`AdminController`を、対象ユーザーごとに分離し、それぞれの責務を明確にしました。
...
- **TagControllerの実装**: タグの編集画面表示・作成・更新・削除を担う`TagController`を実装し、3つのコントローラーによる責務分離を完成させました。
- **`attach()`によるタグ紐付け**: 多対多リレーションの`attach()`メソッドで、お問い合わせとタグを紐付ける方法を学びました。
- **Eager LoadingとN+1問題の回避**: `with()`メソッドでリレーションを事前読み込みし、パフォーマンス問題を防ぐ方法を実践しました。
