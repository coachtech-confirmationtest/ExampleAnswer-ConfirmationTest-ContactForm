...
このチャプターでは、アプリケーションのデータを保存するための「骨格」となるデータベースのテーブルを作成します。具体的には、Laravelの**マイグレーション**という強力な機能を使い、PHPのコードでデータベースの構造を定義し、バージョン管理する方法を学びます。一対多リレーション（Category-Contact）だけでなく、タグ機能のための**多対多リレーション**（Contact-Tag）を実現する**中間テーブル**の設計も含め、全4テーブルのマイグレーションを作成します。これにより、誰が作業しても同じデータベース構造を正確に再現できるようになります。
...
Chapter 0で確認した要件に基づき、今回作成する4つのテーブルの具体的な設計を再確認します。この設計図（マイグレーションファイル）を元に、実際のテーブルを構築していきます。
...
| `tags` | お問い合わせに付与するタグ（例: 「質問」「要望」）を管理するマスターテーブル。 |
| `contact_tag` | `contacts`テーブルと`tags`テーブルの多対多の関係を定義するための中間テーブル。 |
...
| カラム名 | データ型 | 役割と制約 |
| :--- | :--- | :--- |
| `id` | `BIGINT` (PK) | タグを一意に識別するためのID。自動で連番が振られる。 |
| `name` | `VARCHAR(50)` | タグの名前（例：「緊急」「質問」）。重複は許さない（`unique`制約）。 |
| `created_at` | `TIMESTAMP` | レコードが作成された日時。 |
| `updated_at` | `TIMESTAMP` | レコードが更新された日時。 |

### `contact_tag`テーブル（中間テーブル）のカラム定義

`contacts`テーブルと`tags`テーブルの多対多の関係を定義するための中間テーブルです。

| カラム名 | データ型 | 役割と制約 |
| :--- | :--- | :--- |
| `id` | `BIGINT` (PK) | このリレーションの一意なID。 |
| `contact_id` | `BIGINT` (FK) | `contacts`テーブルのIDへの外部キー。`onDelete('cascade')`設定。 |
| `tag_id` | `BIGINT` (FK) | `tags`テーブルのIDへの外部キー。`onDelete('cascade')`設定。 |
| `created_at` | `TIMESTAMP` | 作成日時。 |
| `updated_at` | `TIMESTAMP` | 更新日時。 |
| (`contact_id`, `tag_id`) | - | 同じ問い合わせに同じタグが複数紐づくことを防ぐ複合ユニークキー。 |
...
1.  **`make:migration`**: まず、何をするかの「宣言」として、空の設計図ファイルを作成します。
...
### Point 6: 多対多リレーションには「中間テーブル」が必要

`categories`と`contacts`の関係は「一対多」でした。一方、タグ機能では「一つの問い合わせに複数のタグ」かつ「一つのタグが複数の問い合わせに紐づく」という**多対多（Many-to-Many）**の関係が必要です。リレーショナルデータベースでは一つのカラムに複数の値を格納できないため、`contacts`テーブルに`tag_ids`のようなカラムを作ってカンマ区切りで保存するのはアンチパターンです。代わりに、`contact_tag`という中間テーブルを導入し、各行が「どの問い合わせ」が「どのタグ」に紐づくかという単一の事実を記録します。

### Point 7: 中間テーブルの命名規則とカスケード削除

Laravelには「関連する2つのテーブル名を、単数形で、アルファベット順に並べ、アンダースコアで繋ぐ」という中間テーブルの命名規約があります。`contact`と`tag`をアルファベット順に並べると`contact_tag`です。この規約に従うことで、モデルのリレーション定義で余計な設定が不要になります。また、中間テーブルの外部キーには`onDelete('cascade')`を設定し、問い合わせやタグが削除されたら関連する中間テーブルのレコードも自動的に削除されるようにします。さらに、`unique(['contact_id', 'tag_id'])`の複合ユニークキーで、同じ問い合わせに同じタグが重複して紐づくことを防ぎます。
...
### 4.4. `create_tags_table`マイグレーションの作成と編集

タグ情報を格納するテーブルのマイグレーションを作成します。
...
### 4.5. `create_contact_tag_table`中間テーブルのマイグレーション作成と編集

`contacts`テーブルと`tags`テーブルの多対多の関係を管理する中間テーブルのマイグレーションを作成します。
...
- **`$table->string('name', 50)->unique();`**
  `name`カラムを`VARCHAR(50)`として定義し、`unique`制約を付けています。これにより、同じ名前のタグが複数登録されるのをデータベースレベルで防ぎます。`50`という長さは、タグ名として十分な長さを確保しつつ、無駄に大きな領域を確保しないためのバランスを考えた設定です。

### `..._create_contact_tag_table.php` の解説

- **`$table->foreignId('contact_id')->constrained()->onDelete('cascade');`**
  `contact_id`カラムを作成し、`contacts`テーブルへの外部キー制約とカスケード削除を設定します。`constrained()`はカラム名から親テーブル（`contacts`）を自動推測します。親レコードが削除されると、この中間テーブルの関連レコードも自動削除されます。

- **`$table->foreignId('tag_id')->constrained()->onDelete('cascade');`**
  `contact_id`と同様に、`tags`テーブルへの外部キー制約とカスケード削除を設定します。

- **`$table->unique(['contact_id', 'tag_id']);`**
  「複合ユニークキー」を設定しています。`contact_id`と`tag_id`の組み合わせが常に一意であることを保証します。これにより、例えば「ID:1の問い合わせ」に「ID:5のタグ」が2回以上紐付けられる、といったデータの重複を防ぎます。
...
3. `tags`テーブルが作成されていること
   - カラム: `id`, `name`(unique), `created_at`, `updated_at`
4. `contact_tag`テーブルが作成されていること
   - カラム: `id`, `contact_id`, `tag_id`, `created_at`, `updated_at`
   - `contact_id`と`tag_id`に外部キー制約が設定されていること
   - `contact_id`と`tag_id`の組み合わせにユニーク制約が設定されていること
...
- **多対多リレーションと中間テーブル**: 一つの問い合わせが複数のタグを持ち、一つのタグが複数の問い合わせに紐づく関係性を、`contact_tag`中間テーブルで実現する方法を学びました。
- **複合ユニークキー**: `unique(['contact_id', 'tag_id'])`でデータの重複を防ぎ、外部キー制約とカスケード削除でデータの整合性を保つ方法を理解しました。
- **マイグレーションの実装**: `make:migration`でファイルを作成し、スキーマビルダでカラムを定義し、`migrate`で実行するという一連の流れを実践しました。

これで、アプリケーションがデータを保存するための「器」が用意できました。しかし、まだ器は空っぽで、プログラムから操作する方法もありません。次のチャプターでは、Category、Contact、Tagの3つの**モデル**とそれらの間の**リレーション**を定義していきます。
